...
namespace wit_imu_driver
{
    ...
    public:
        WitImuDriver(rclcpp::Node::SharedPtr node)
            : node_(node), port_io_(), port_(port_io_), wdg_timeout_(0.5s), rx_buf_(1024)
        {
            ...
            node_->declare_parameter("baud", 115200);
        }
        ...
        bool open()
        {
            ...
            port_.set_option(ba::serial_port_base::parity(
                                 ba::serial_port_base::parity::none),
                             ec);
            if (ec.value() != 0)
            {
                RCLCPP_ERROR(node_->get_logger(),
        ...
        bool spin()
        {
            ...
            // wdg_ = node_->create_wall_timer(wdg_timeout_, std::bind(&WitImuDriver::cbWdg, this));
            // wdg_ = node_->create_wall_timer(wdg_timeout_, [this]() { this->cbWdg(); });

            // wdg_ = node_->create_wall_timer(
            //     std::chrono::milliseconds(static_cast<int>(wdg_timeout_ * 1000)),
            //     [this]() { this->cbWdg(); });
            // wdg_ = node_->create_wall_timer(
            //     std::chrono::milliseconds(wdg_timeout_.seconds() * 1000),
            //     [this]() { this->cbWdg(); });

            wdg_ = node_->create_wall_timer(
                std::chrono::milliseconds(static_cast<long int>(wdg_timeout_.seconds() * 1000)),
                [this]() { this->cbWdg(); });
    ...
    private:
                    ...
                    std::size_t size)
        {
            ...
            {
                RCLCPP_ERROR(node_->get_logger(), "Serial error: %s", ec.message().c_str());
                rclcpp::shutdown();
            }
        }
        ...
        void cbWdg()
        {
            if (port_.is_open())
            {
                RCLCPP_ERROR(node_->get_logger(), "Timeout. No data received from IMU.");
                rclcpp::shutdown();
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (!imu.spin())
    {
        RCLCPP_ERROR(rclcpp::get_logger("wit_imu_driver"), "Failed to spin.");
        return 1;
    }
    return 0;
}
