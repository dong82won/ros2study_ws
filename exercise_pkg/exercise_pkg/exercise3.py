...
# MultiThreadedExecutor()는 여러 개의 스레드를 사용하여 노드를 실행할 수 있도록 해줍니다.
# MutuallyExclusiveCallbackGroup()은 서로 배타적인 콜백 그룹을 생성하여, 동시에 실행될 수 있는 콜백을 제한합니다.
# 이 조합을 사용하면, 여러 개의 콜백이 동시에 실행될 수 있지만, 각 콜백은 서로 배타적으로 실행됩니다.
# 즉, 한 콜백이 실행되는 동안 다른 콜백은 실행되지 않습니다.
# 이 조합은 멀티스레딩을 사용하여 노드를 실행하면서도, 각 콜백이 서로 배타적으로 실행되도록 보장합니다.
